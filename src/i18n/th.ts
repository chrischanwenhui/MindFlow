export const th = {
  langLabel: 'ภาษา',
  langEnglish: 'English',
  langChinese: '简体中文',
  langMalay: 'Bahasa Melayu',
  langJapanese: '日本語',
  langKorean: '한국어',
  langThai: 'ไทย',
  navBrand: 'MindFlow', navAbout: 'เกี่ยวกับ', navProvide: 'สิ่งที่เราให้', navSaveExit: 'บันทึกและออก', navAssessment: 'แบบประเมิน', navAboutMindflow: 'เกี่ยวกับ MindFlow',
  landingTitle: 'Mindflow', landingByline: 'by Eirene Stack',
  landingDesc: 'แบบประเมินเพื่อการค้นพบตนเอง สำหรับสัญญาณการสะท้อนตน การประมาณลักษณะ ทิศทางอาชีพ และการตระหนักรู้สไตล์การคิด',
  begin: 'เริ่ม', startTitle: 'เริ่มแบบประเมิน', startEstimate: 'เวลาโดยประมาณ: 12–18 นาที รายงานนี้ไม่ใช่การวินิจฉัย และใช้เพื่อการสะท้อนตนเท่านั้น',
  sessionCountNotice: 'คุณจะตอบคำถาม {count} ข้อในเซสชันนี้',
  sessionOrderNotice: 'คำถามบุคลิกภาพและสไตล์การทำงานมาก่อน คำถามด้านการคิดจะอยู่ช่วงท้าย',
  sessionSaveNotice: 'ความคืบหน้าของคุณจะถูกบันทึกไว้ในอุปกรณ์นี้',
  startQuestions: 'เริ่มคำถาม', resumeSaved: 'ทำต่อจากที่บันทึกไว้', noSaved: 'ยังไม่พบคำตอบที่บันทึกไว้ เริ่มแบบประเมินใหม่ได้เลย',
  progressAria: 'ความคืบหน้าแบบประเมิน', memoryTitle: 'ความท้าทายด้านความจำ', memoryIntro: 'คุณจะเห็นลำดับข้อมูลไม่กี่วินาที พอหายไปแล้วให้ตอบโดยไม่ย้อนดู', memoryReadyTitle: 'ความท้าทายด้านความจำ', memoryReadyBody: 'คุณจะเห็นลำดับสั้น ๆ กด พร้อม เมื่อคุณต้องการเริ่มแสดง', readyReveal: 'พร้อม', memoryHiddenNotice: 'ลำดับความจำถูกซ่อนไว้แล้ว กรุณาตอบจากความจำ', timeRemaining: 'เวลาที่เหลือ', timeUpNotice: 'หมดเวลาแล้ว — ยังตอบได้ แต่จะถือว่าเป็นคำตอบล่าช้า', memoryReady: 'พร้อม — แสดงลำดับ', memorizeThis: 'จดจำสิ่งนี้', needHelp: 'ต้องการความช่วยเหลือในการเข้าใจคำถามไหม?', cognitiveNoBackNotice: 'คำถามด้านการคิดจะไม่แสดงถูก/ผิดระหว่างการทำแบบประเมิน', memoryProtectionTitle: 'การปกป้องคำถามความจำ', memoryProtectionBody: 'เพื่อความยุติธรรม คำถามความจำที่ถูกเปิดแล้วจะไม่สามารถย้อนกลับไปดูได้ หากย้อนกลับ ระบบจะสร้างคำถามความจำข้อใหม่และล็อกคำตอบเดิมของคุณ', regenerateMemoryQuestion: 'ดำเนินการต่อ', memoryPoolExhausted: 'ชุดคำถามความจำหมดแล้วสำหรับเซสชันนี้',  navCancel: 'ยกเลิก', backPrev: 'กลับไปคำถามก่อนหน้า',
  resultsTitle: 'สรุปผลลัพธ์', personalityEstimate: 'การประมาณประเภทบุคลิกภาพ:', motivationPattern: 'รูปแบบแรงจูงใจ:', viewReport: 'ดูตัวอย่างรายงาน', reportPreview: 'ตัวอย่างรายงานการค้นพบตนเอง', printPdf: 'พิมพ์หรือบันทึกเป็น PDF',
  aboutTitle: 'เกี่ยวกับ MindFlow', provideTitle: 'สิ่งที่เราให้', restart: 'เริ่มใหม่',
  translationNotice: 'ประกาศคุณภาพการแปล: UI หลายภาษานี้ยังเป็นฉบับร่างที่พัฒนาอย่างต่อเนื่อง บางถ้อยคำอาจปรับปรุงเพิ่มเติมเพื่อความชัดเจนในท้องถิ่น',
  localSaveNotice: 'ความคืบหน้าจะถูกบันทึกไว้ในอุปกรณ์/เบราว์เซอร์นี้เท่านั้น',
  heroKicker: 'MindFlow • Eirene Stack',
  nonDiagnosticNotice: 'ประสบการณ์นี้ออกแบบเพื่อการค้นพบตนเองและการสะท้อนตน ไม่ใช่การประเมินทางคลินิก ทางการแพทย์ ทางวินิจฉัย หรือการทดสอบ IQ อย่างเป็นทางการ',
  assessmentCategoriesAriaLabel: 'Assessment categories',
  assessmentCategoriesDisclaimer: 'This assessment includes the following reflection categories:',
  categoryPersonalityTitle: 'Personality & Preferences',
  categoryPersonalityDesc: 'Estimate your likely trait and preference signals from response patterns.',
  categoryCareerTitle: 'Career Interests',
  categoryCareerDesc: 'Highlight RIASEC-style interest directions to support role exploration.',
  categoryCognitiveTitle: 'Cognitive Style',
  categoryCognitiveDesc: 'Reflect on non-diagnostic reasoning and memory-style tendencies.',
  categoryStrengthsTitle: 'Strengths & Growth',
  categoryStrengthsDesc: 'Surface strengths, watch-outs, and practical growth areas to test.',
  categoryPatternsTitle: 'Motivation Patterns',
  categoryPatternsDesc: 'Summarize motivation, stress, and leadership response patterns.',
  categoryWorkLearningTitle: 'Work & Learning',
  categoryWorkLearningDesc: 'Connect your style signals to work habits and learning approaches.',
  personalitySection: 'การประมาณประเภทบุคลิกภาพ', riasecSection: 'ความสนใจอาชีพ RIASEC', motivationSection: 'รูปแบบแรงจูงใจ', stressSection: 'แนวโน้มความเครียด', leadershipSection: 'แนวโน้มภาวะผู้นำ', workstyleSection: 'แนวโน้มสไตล์การทำงาน',
  executiveSummarySection: 'Executive Summary', workOperatingSection: 'Stress and Leadership Tendencies', combinedInsightsSection: 'Combined Insights', confidenceSection: 'Disclaimer / Confidence Note', cognitiveSection: 'สรุปสไตล์การคิด',
  strengthsSection: 'จุดแข็ง', blindSpotsSection: 'จุดบอด', growthAreasSection: 'พื้นที่เติบโต',
  stressLabel: 'แนวโน้มความเครียดที่ถูกเลือกมากที่สุด:', leadershipLabel: 'แนวโน้มภาวะผู้นำที่ถูกเลือกมากที่สุด:', workstyleLabel: 'แนวโน้มสไตล์การทำงานที่ถูกเลือกมากที่สุด:',
  motivationSuffix: 'คือสัญญาณการสะท้อนตนที่ประเมินว่าเด่นที่สุดของคุณในตอนนี้',
  executiveSummaryTemplate: 'Your response pattern suggests a {personalityTypeEstimate}-leaning preference estimate with the clearest Big Five signal in {topBigFive}. In this session, your strongest career-interest signal leaned toward {topRiasec}, while your operating-style responses leaned toward {topOperating}. This may indicate a meaningful pattern in how you approach decisions, and your clearest cognitive-style signal in this reasoning sample appeared in {topCognitiveLabel}. A useful reflection point is to treat these signals as prompts for experimentation rather than fixed identity labels.',
  confidenceNoteTemplate: 'Confidence: {confidenceLevel}. This section is based on your current response set, and MindFlow remains a reflective tool rather than a validated psychometric instrument.',
  personalityEstimateSentence: 'Your estimated personality type signal in this session is',
  cognitiveLightSummaryTemplate: 'Your strongest cognitive-style signal in this session appeared in {topCognitiveLabel}. Your cognitive-style result is still a light signal because this is a short, non-diagnostic reasoning sample.',
  cognitiveStandardSummaryTemplate: 'Your strongest cognitive-style signal in this session appeared in {topCognitiveLabel}. This non-diagnostic reasoning sample reflects cognitive-style tendencies such as working memory and problem framing, not any official intelligence ranking.',
  combinedInsightMilestones: 'You may work best when goals are broken into visible milestones and progress can be tracked clearly.',
  combinedInsightInvestigativeOpen: 'You may be drawn to roles where analysis and new frameworks intersect.',
  combinedInsightStressControl: 'Under uncertainty, you may regain stability by turning ambiguity into a concrete next action.',
  combinedInsightSocialFacilitative: 'You may contribute strongly in roles requiring trust-building, coaching, or team alignment.',
  combinedInsightWorkflowAdjustments: 'In this session, your cross-domain signals suggest testing small workflow adjustments and reviewing what improves focus, energy, and follow-through.',
  combinedInsightCrossDomainReview: 'A useful reflection point is to compare where you feel most effective with how your strongest signals appear across personality, motivation, and cognitive-style responses.',
  mbtiBestFitTemplate: 'รูปแบบคำตอบปัจจุบันของคุณเอนเอียงไปทาง {personalityTypeEstimate} ระดับความมั่นใจ: {confidenceLevel}',
  personalitySignalDisclaimer: 'คะแนนเหล่านี้เป็นสัญญาณเพื่อการทบทวนตนเองจากคำตอบของคุณ ไม่ใช่ป้ายกำกับตัวตนที่ตายตัว',
  scoreBarReflectionHint: 'โปรดมองสิ่งนี้เป็นสัญญาณเพื่อการทบทวนตนเอง ไม่ใช่ตัวตนที่ตายตัว',
  mbtiDimensionDetailTemplate: '{leanStrength} ไปทาง {poleLabel} ({signalLabel})',
  mbtiDimensionEnergyFocus: 'พลังงานและจุดโฟกัส',
  mbtiDimensionInformationStyle: 'สไตล์การรับข้อมูล',
  mbtiDimensionDecisionAnchor: 'หลักยึดในการตัดสินใจ',
  mbtiDimensionStructurePreference: 'ความชอบด้านโครงสร้าง',
  mbtiSlightLean: 'สัญญาณต่ำ / หลักฐานผสม',
  mbtiModerateLean: 'เอนเอียงระดับปานกลาง',
  mbtiStrongLean: 'เอนเอียงชัดเจน',
  mbtiLowSignal: 'สัญญาณต่ำ / หลักฐานผสม',
  mbtiModerateSignal: 'สัญญาณปานกลาง',
  mbtiStrongSignal: 'สัญญาณชัดเจน',
  mbtiPoleE: 'การมุ่งออกสู่ภายนอก',
  mbtiPoleI: 'การมุ่งสู่ภายใน',
  mbtiPoleS: 'การรับรู้จากสิ่งที่เป็นรูปธรรม',
  mbtiPoleN: 'สัญชาตญาณ',
  mbtiPoleT: 'การใช้เหตุผล',
  mbtiPoleF: 'การใช้ความรู้สึก',
  mbtiPoleJ: 'การจัดระบบ',
  mbtiPoleP: 'การเปิดรับความเป็นไปได้',
  cognitiveSkipReassurance: 'Skipping difficult questions is okay. MindFlow looks at patterns across the full session, not perfection.',
  cognitiveUnknownNotice: 'การเลือก “ไม่ทราบ” จะถือเป็นสัญญาณการให้เหตุผลที่ยังไม่ตอบ ไม่ใช่การหักคะแนน',
  forcedChoiceHelper: 'Choose the option closer to your usual first instinct. Both can be useful.',
  studyThisSequence: 'Study this item'
  // TODO: Executive summary translations can be refined in a dedicated localization PR.
} as const;
