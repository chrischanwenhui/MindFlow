...
const mkCog = (id:string, d:CognitiveDomain, difficulty:CognitiveDifficulty, prompt:string, opts:[string,number][], extra:Partial<Question>={}):Question => ({ id, section:'cognitive', cognitiveDomain:d, difficulty, hint:COG_HINTS[d], prompt, options:[...opts.map(([label,score])=>({label:label.trim().length < 2 ? `Value ${label}` : label,value:d,score})), {label:"I don't know", value:'default-idk', score:0}], scoringDomain:d, groupLabel:'Cognitive Style', ...extra });
...
const ocean = (Object.entries(oceanSeeds) as [keyof typeof oceanSeeds, readonly string[]][]).flatMap(([trait, prompts]) =>
  prompts.map((prompt, i) => ({ id:`ocean-${trait === 'conscientiousness' ? 'cons' : trait === 'extraversion' ? 'extra' : trait === 'agreeableness' ? 'agree' : trait === 'neuroticism' ? 'neuro' : 'open'}-${i+1}`, section:'ocean' as const, prompt, options:likert(trait), scoringDomain:trait, scoringDirection:([2,3,5,7].includes(i) ? 'reverse':'positive') as 'positive'|'reverse', groupLabel:'Big Five / OCEAN' }))
);

const riasecTriples:[string,[string,string,string]][] = [
['Which task would you volunteer for in a community workshop?',['Set up tools and fix equipment','Review the data and diagnose causes','Design the event visuals and theme']],
['In a new project, which part sounds most satisfying?',['Build a working prototype','Test assumptions with evidence','Craft the story and presentation']],
['Which side project would you start first?',['Restore a bike or device','Analyze market trends','Write songs or short scripts']],
['What role feels most natural during problem solving?',['Hands-on troubleshooter','Research lead','Concept artist']],
['Which activity would feel most energizing this weekend?',['Woodworking or home repairs','Learning a technical topic deeply','Photography or illustration']],
['Which class would you take by choice?',['Practical mechanics lab','Statistics and experiment design','Creative writing studio']],
['At work, where do you often add value first?',['Executing concrete tasks quickly','Finding patterns in complex information','Generating original concepts']],
['What kind of challenge keeps your attention longest?',['Operating machinery safely','Solving analytical puzzles','Producing expressive work']],
['Which contribution would you make in a team event?',['Coach a beginner one-on-one','Facilitate a group discussion','Pitch ideas to sponsors']],
['Which activity sounds most fulfilling after work?',['Mentor someone who is learning','Organize a neighborhood meetup','Lead a small fundraiser effort']],
['In group work, where do you naturally step in?',['Support people who feel stuck','Help align everyone’s priorities','Coordinate external partners']],
['Which role do others often ask you to take?',['Trainer or helper','Team coordinator','Spokesperson or negotiator']],
['What kind of impact feels most meaningful?',['Helping people build confidence','Creating fair systems for teams','Influencing decisions at scale']],
['Which volunteer task would you choose?',['Peer coaching','Mediating between groups','Leading outreach calls']],
['During conflict, what approach fits you best?',['Listen and support each side','Structure a workable process','Persuade others toward a direction']],
['Which workstream sounds appealing in a startup?',['Client support and onboarding','Process documentation','Business development']],
['Which desk activity is most satisfying to you?',['Maintain orderly records','Track budgets and timelines','Standardize procedures']],
['Which project responsibility do you prefer?',['Quality checks and compliance','Spreadsheet planning','Creating reusable templates']],
['When handling details, what do you enjoy most?',['Accurate filing and categorization','Coordinating schedules and dependencies','Improving workflow consistency']],
['Which environment helps you do your best work?',['Clear rules and stable routines','Structured targets and measurable progress','Defined processes and reliable systems']]
];
const riasec = riasecTriples.map(([prompt, labels], i) => ({ id:`riasec-${i+1}`, section:'riasec' as const, prompt, options: labels.map((label,idx)=>({label, value:['Realistic','Investigative','Artistic','Social','Enterprising','Conventional'][Math.floor(i/3)+(idx===0?0:idx===1?1:2)] ?? 'Realistic', score:2})), scoringDomain:'riasec', groupLabel:'RIASEC Interests' }));
...
while (mslw.length < 32) {
  const idx = mslw.length + 1;
  const section = (['motivation','stress','leadership','workstyle'] as Section[])[mslw.length % 4];
  const promptMap: Record<string,string> = {
    motivation: 'Which condition helps you sustain quality effort over months?',
    stress: 'When uncertainty rises, which response is most typical for you?',
    leadership: 'When coordinating others, what do you emphasize first?',
    workstyle: 'In day-to-day execution, which approach fits you best?'
  };
  const optionMap: Record<string,[string,string,string][]> = {
    motivation: [['Clear growth milestones','growth','2'],['Autonomy in choosing methods','autonomy','2'],['Shared commitment with the team','shared','2']],
    stress: [['Define the next concrete action','action','2'],['Reframe the issue before reacting','reframe','2'],['Seek perspective from a colleague','perspective','2']],
    leadership: [['Set expectations and decision rules','structure','2'],['Coach strengths and remove blockers','coach','2'],['Create buy-in around direction','alignment','2']],
    workstyle: [['Plan deeply before execution','planner','2'],['Iterate quickly and adjust','iterative','2'],['Alternate focus and collaboration','hybrid','2']]
  };
  mslw.push(mkMS(`mslw-${section}-${idx}`, section, `${promptMap[section]} (${Math.ceil(idx/4)})`, optionMap[section]));
}
